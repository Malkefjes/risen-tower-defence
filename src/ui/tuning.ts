...
const SECTIONS: Section[] = [
  ...
  { title: "Enemies (all types)", knobs: [
    ...
    top("enemyHpStep", "HP added each raid (share of raid 1)", 0, 1, 0.01),
    ...
    top("activeCaves", "Caves sending enemies (at most)", 1, 12, 1),
    top("caveEvery", "Another cave opens every N raids (0 = all at once)", 0, 10, 1),
  ] },
  ...enemySections(),
  { title: "Base", knobs: [
    ...
    top("decayTime", "Decay: seconds from full to broken", 10, 3600, 10),
    top("repairTime", "Upkeep mends walls: seconds from broken to full (calm only)", 10, 3600, 10),
