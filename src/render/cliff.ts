...
/**
 * Cliffs: the edges of raised ground (Erik's pick, look B from the cliff round).
 * Each cell is a stack of tilted rock slabs, like frost-split strata, with snow
 * settled evenly on every slab's upper face (the top slab wears a full blanket),
 * drifts against the open faces and a little rubble at the foot.
 */
...
/** Cliff cells, merged per material. `isCliff` says which neighbours are cliff too (the others get drifts). */
export function cliffModel(cells: readonly Cell[], isCliff: (x: number, y: number) => boolean, seed = 1): THREE.Group {
  M ??= palette();
