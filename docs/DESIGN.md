...
- Walls may fully enclose anything (enemies chew through); only terrain can truly seal a path. Numbers to start: stone wall 300 HP per cell, plating ×3, at most 2 enemies claw one wall cell at a time (2 HP/s each), so a stone cell takes about 75 s and a plated one about 4 minutes. A broken cell leaves its piece. **Repair** a damaged piece for stone (its share of the wall price for the HP missing) from the wall's right-click wheel, any time.
