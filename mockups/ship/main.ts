...
// Ship mockup: three designs for the player's ship on a 3×3 footprint, in the
...
const SHIPS: ShipDesign[] = [
  {
    key: "A", name: "Lander",
    text: "Squat and wide on four landing legs, white hull with an orange band, the engine core caged on top. Reads as \"we just touched down\", and stays low enough to see over.",
    build() {
      const g = new THREE.Group();
      // Legs: angled struts to round pads at the corners of the 3×3.
      for (const [sx, sz] of [[1, 1], [1, -1], [-1, 1], [-1, -1]] as const) {
        const pad = mesh(new THREE.CylinderGeometry(0.2, 0.24, 0.06, 10), M.steelDark, sx * 1.22, 0.03, sz * 1.22);
        // Strut from the pad up to the hull's underside.
        const foot = new THREE.Vector3(sx * 1.22, 0.05, sz * 1.22), hip = new THREE.Vector3(sx * 0.75, 0.62, sz * 0.75);
      ...
      g.add(mesh(new THREE.CylinderGeometry(1.1, 0.95, 0.2, 8).rotateY(Math.PI / 8), M.steelDark, 0, 0.42, 0));
      g.add(mesh(new THREE.CylinderGeometry(1.25, 1.2, 0.55, 8).rotateY(Math.PI / 8), M.hull, 0, 0.8, 0));
      g.add(mesh(new THREE.CylinderGeometry(1.27, 1.27, 0.12, 8).rotateY(Math.PI / 8), M.orange, 0, 0.72, 0));
      g.add(mesh(new THREE.CylinderGeometry(0.95, 1.2, 0.22, 8).rotateY(Math.PI / 8), M.hullShade, 0, 1.18, 0));
      // Cockpit visor facing the camera corner.
      const visor = box(0.6, 0.14, 0.08, M.visor, 0, 1.12, 0);
      visor.position.set(0.62, 1.2, 0.62); visor.rotation.y = Math.PI / 4;
      g.add(visor);
      const core = engineCore(1.1);
      core.group.position.y = 1.29;
      g.add(core.group);
      const bay = cargoBay(0.8, 0.42, 1.17, 0.52);
      g.add(bay);
      g.add(fabricator(1.17, 0.62, 0.7).group);
      g.add(box(0.05, 0.5, 0.05, M.steelLight, -0.7, 1.29, -0.5), box(0.14, 0.04, 0.14, M.orange, -0.7, 1.79, -0.5));
      return { group: shadowAll(g), update: core.update, height: 2.4 };
    },
  },
  {
    key: "B", name: "Hauler",
    text: "A long industrial cargo ship on skids. Twin engines at the back with the core glowing inside its housing, a big clamshell cargo door at the front. Reads as a mining vessel.",
    build() {
      const g = new THREE.Group();
      for (const sx of [-1, 1]) g.add(box(0.16, 0.12, 2.8, M.steelDark, sx * 0.95, 0, 0));
      for (const sx of [-1, 1]) for (const sz of [-1, 1]) g.add(box(0.1, 0.3, 0.1, M.steelLight, sx * 0.95, 0.1, sz * 0.9));
      g.add(rbox(2.2, 0.85, 2.7, 0.14, M.hull, 0, 0.38, 0));
      g.add(box(2.24, 0.12, 2.74, M.orange, 0, 0.62, 0));
      g.add(box(2.1, 0.12, 2.6, M.steelDark, 0, 0.3, 0));
      // Cockpit hump at the front.
      g.add(rbox(1.2, 0.34, 0.8, 0.1, M.hullShade, 0, 1.2, 0.7));
      g.add(box(0.9, 0.12, 0.06, M.visor, 0, 1.32, 1.1));
      // Engine housing at the back with the core inside, open on top.
      const housing = new THREE.Group();
      housing.add(mesh(new THREE.CylinderGeometry(0.7, 0.75, 0.3, 12), M.steel, 0, 0.15, 0));
      housing.add(mesh(new THREE.TorusGeometry(0.66, 0.06, 6, 24).rotateX(Math.PI / 2), M.orange, 0, 0.3, 0));
      const core = engineCore(1.0, 6);
      core.group.position.y = 0.1;
      housing.add(core.group);
      housing.position.set(0, 1.2, -0.55);
      g.add(housing);
      for (const sx of [-1, 1]) {
        const nacelle = mesh(new THREE.CylinderGeometry(0.26, 0.3, 0.9, 10).rotateX(Math.PI / 2), M.steel, sx * 1.22, 0.9, -0.85);
        const nozzle = mesh(new THREE.CylinderGeometry(0.2, 0.26, 0.12, 10).rotateX(Math.PI / 2), M.power, sx * 1.22, 0.9, -1.36);
        g.add(nacelle, nozzle, box(0.1, 0.08, 0.6, M.orange, sx * 1.22, 1.12, -0.85));
      }
      g.add(cargoBay(1.3, 0.5, 1.37, 0.4));
      g.add(fabricator(1.12, 0.52, 0.9).group);
      return { group: shadowAll(g), update: core.update, height: 2.3 };
    },
  },
  {
    key: "C", name: "Command hub",
    text: "A round modular hub on a steel ring, with the engine core rising from its center and bays around it: cargo in front, fabricator to the side, a habitat module behind. Reads most like a base, least like a vehicle.",
    build() {
      const g = new THREE.Group();
      g.add(mesh(new THREE.CylinderGeometry(1.42, 1.48, 0.22, 16), M.steelDark, 0, 0.11, 0));
      g.add(mesh(new THREE.CylinderGeometry(1.0, 1.08, 0.75, 16), M.hull, 0, 0.6, 0));
      g.add(mesh(new THREE.CylinderGeometry(1.1, 1.1, 0.1, 16), M.orange, 0, 0.3, 0));
      g.add(mesh(new THREE.CylinderGeometry(0.72, 1.0, 0.2, 16), M.hullShade, 0, 1.07, 0));
      // Bay modules around the hub.
      g.add(rbox(0.95, 0.62, 0.7, 0.08, M.orange, 0, 0.22, 0.95));
      g.add(rbox(0.7, 0.62, 0.95, 0.08, M.hull, 0.95, 0.22, 0));
      g.add(rbox(0.8, 0.5, 0.8, 0.08, M.orangeDark, -0.8, 0.22, -0.8));
      g.add(box(0.5, 0.1, 0.06, M.visor, -0.55, 0.55, -0.4).rotateY(Math.PI / 4));
      // Connector collars between the hub and the bays.
      g.add(box(0.5, 0.3, 0.2, M.steel, 0, 0.35, 0.62), box(0.2, 0.3, 0.5, M.steel, 0.62, 0.35, 0));
      const core = engineCore(1.15);
      core.group.position.y = 1.17;
      g.add(core.group);
      g.add(cargoBay(0.62, 0.36, 1.31, 0.24));
      g.add(fabricator(1.31, 0.3, 0.6).group);
      // Antenna mast.
      g.add(box(0.05, 0.9, 0.05, M.steelLight, -0.9, 0.72, -0.9), box(0.16, 0.04, 0.16, M.orange, -0.9, 1.62, -0.9));
      return { group: shadowAll(g), update: core.update, height: 2.4 };
    },
  },
];
...
function setFocus(i: number): void {
  ...
  if (i < 0) {
    ...
    wantTarget.set(c.x - 0.3, 0, c.z + 0.2); wantZoom = 3.6;
    const d = SHIPS[i]!;
