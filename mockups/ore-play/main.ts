...
const NODE_YIELD: Record<NodeKind, number> = { stone: 600, metal: 300 };
interface Node {
...
const ring = new THREE.Sprite(new THREE.SpriteMaterial({ map: ringTex, color: "#ffffff", transparent: true, depthWrite: false, depthTest: false, blending: THREE.AdditiveBlending }));
ring.visible = false;
ring.renderOrder = 5;
scene.add(ring);
const IDLE_TINT = new THREE.Color("#ffffff"), HIT_TINT = new THREE.Color("#ffffff");
...
/** A spark (white burst when it comes off the hotspot), or a small chip of the node's material. */
const cyanSparkMat = new THREE.MeshBasicMaterial({ color: "#ffffff" });
...
function frame(now: number): void {
  ...
  // Two clear states: idle (small, soft, see-through) and targeted (full bright, bigger, spinning, with a ring).
