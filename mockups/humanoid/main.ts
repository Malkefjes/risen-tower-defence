...
// Player rig mockup: three humanoid exo-rigs for the player, next to the real Rocket, each running
...
const DESIGNS: Design[] = [
  {
    key: "A",
    text: "A compact exo-rig: a broad white chest with a glass canopy where you see the pilot's helmet, big orange shoulder plates, a small sensor head with a cyan eye, and an ore rack on the back. Chunky and readable.",
    build() {
      const sk = skeleton(
        { hipY: 0.36, hipW: 0.09, thigh: 0.17, shin: 0.17, legW: 0.09, shoulderY: 0.66, shoulderW: 0.2, upperArm: 0.15, foreArm: 0.15, armW: 0.075, foot: [0.1, 0.05, 0.17] },
        { armor: M.suit, limb: M.steel, joint: M.steelDark, accent: M.orange },
      );
      const b = sk.body;
      b.add(rbox(0.16, 0.08, 0.13, 0.03, M.steelDark, 0, 0.35, 0));
      b.add(rbox(0.32, 0.28, 0.24, 0.07, M.suit, 0, 0.42, 0));
      const canopy = mesh(new THREE.SphereGeometry(0.1, 12, 8, -Math.PI * 0.4, Math.PI * 0.8, Math.PI * 0.25, Math.PI * 0.45), M.visor, 0, 0.58, 0.07);
      b.add(canopy);
      b.add(mesh(new THREE.SphereGeometry(0.045, 8, 6), M.suitShade, 0, 0.6, 0.09));
      for (const sx of [-1, 1]) b.add(rbox(0.13, 0.08, 0.2, 0.03, M.orange, sx * 0.2, 0.66, 0));
      b.add(rbox(0.1, 0.07, 0.09, 0.025, M.steelLight, 0, 0.72, -0.02), box(0.06, 0.02, 0.02, M.power, 0.01, 0.74, 0.025));
      b.add(box(0.2, 0.02, 0.02, M.power, 0, 0.46, 0.125));
      b.add(box(0.24, 0.22, 0.05, M.steelDark, 0, 0.44, -0.15));
      const load = [0, 1, 2].map(i => { const c = oreChunk(0.05); c.position.set((i - 1) * 0.07, 0.58 + (i % 2) * 0.03, -0.2); b.add(c); return c; });
      return { root: sk.root, body: b, load, walk: sk.walk, mine: sk.mine, speed: 1.6, height: 1.0 };
    },
  },
  {
    key: "B",
    text: "A slim power suit, the most human: a helmet with a wide cyan visor, white armor plates over dark steel joints, orange on the forearms and shins, and a slim power pack that holds the ore. Taller and lighter on its feet.",
    build() {
      const sk = skeleton(
        { hipY: 0.44, hipW: 0.07, thigh: 0.22, shin: 0.21, legW: 0.07, shoulderY: 0.78, shoulderW: 0.14, upperArm: 0.18, foreArm: 0.17, armW: 0.06, foot: [0.08, 0.045, 0.15] },
        { armor: M.suit, limb: M.steelDark, joint: M.steel, accent: M.orange },
      );
      const b = sk.body;
      b.add(rbox(0.16, 0.1, 0.12, 0.03, M.steelDark, 0, 0.42, 0));
      b.add(rbox(0.24, 0.26, 0.17, 0.06, M.suit, 0, 0.52, 0));
      b.add(box(0.12, 0.08, 0.02, M.suitShade, 0, 0.6, 0.09));
      for (const sx of [-1, 1]) b.add(rbox(0.09, 0.06, 0.13, 0.025, M.suitShade, sx * 0.14, 0.76, 0));
      b.add(box(0.07, 0.05, 0.07, M.steelDark, 0, 0.78, 0));
      b.add(mesh(new THREE.SphereGeometry(0.09, 14, 10), M.suit, 0, 0.9, 0));
      const visor = mesh(new THREE.SphereGeometry(0.075, 14, 8, -Math.PI * 0.42, Math.PI * 0.84, Math.PI * 0.35, Math.PI * 0.28), M.power, 0, 0.9, 0.025);
      b.add(visor);
      b.add(rbox(0.16, 0.22, 0.07, 0.03, M.steel, 0, 0.5, -0.12), box(0.04, 0.14, 0.01, M.power, 0.05, 0.55, -0.16));
      const load = [0, 1, 2].map(i => { const c = box(0.1, 0.045, 0.02, M.ore, -0.02, 0.54 + i * 0.055, -0.162); b.add(c); return c; });
      // Orange on the forearms and shins.
      for (const a of sk.arms) a.mid.add(box(0.07, 0.05, 0.07, M.orange, 0, -0.1, 0));
      for (const l of sk.legs) l.mid.add(box(0.075, 0.06, 0.075, M.orange, 0, -0.1, 0.005));
      return { root: sk.root, body: b, load, walk: sk.walk, mine: sk.mine, speed: 1.8, height: 1.15 };
    },
  },
  {
    key: "C",
    text: "A heavy exo-rig: wide shoulders, thick arms and short sturdy legs, with the pilot sitting in a dark glass bubble for a head. Orange stripes down the chest, twin thrusters on the back and an ore bin between them. Strongest silhouette.",
    build() {
      const sk = skeleton(
        { hipY: 0.33, hipW: 0.12, thigh: 0.15, shin: 0.16, legW: 0.11, shoulderY: 0.64, shoulderW: 0.25, upperArm: 0.16, foreArm: 0.17, armW: 0.1, foot: [0.13, 0.06, 0.2] },
        { armor: M.suit, limb: M.steel, joint: M.steelDark, accent: M.orange },
      );
      const b = sk.body;
      b.add(rbox(0.22, 0.09, 0.16, 0.03, M.steelDark, 0, 0.32, 0));
      b.add(rbox(0.4, 0.26, 0.26, 0.08, M.suit, 0, 0.4, 0));
      for (const sx of [-1, 1]) b.add(box(0.03, 0.24, 0.02, M.orange, sx * 0.07, 0.41, 0.13));
      b.add(box(0.12, 0.025, 0.02, M.power, 0, 0.6, 0.13));
      for (const sx of [-1, 1]) b.add(rbox(0.15, 0.1, 0.22, 0.04, M.steel, sx * 0.25, 0.64, 0), box(0.155, 0.03, 0.225, M.orange, sx * 0.25, 0.7, 0));
      // Bubble head with the pilot inside.
      b.add(mesh(new THREE.CylinderGeometry(0.1, 0.12, 0.04, 14), M.steelDark, 0, 0.68, 0));
      const bubble = mesh(new THREE.SphereGeometry(0.11, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.6), M.visor, 0, 0.7, 0);
      b.add(bubble, mesh(new THREE.SphereGeometry(0.045, 8, 6), M.suit, 0, 0.75, 0.01));
      // Twin thrusters and the ore bin between them.
      for (const sx of [-1, 1]) b.add(mesh(new THREE.CylinderGeometry(0.045, 0.055, 0.2, 10), M.steel, sx * 0.1, 0.46, -0.17), mesh(new THREE.CircleGeometry(0.04, 10).rotateX(Math.PI / 2), M.power, sx * 0.1, 0.355, -0.17));
      b.add(box(0.1, 0.12, 0.08, M.steelDark, 0, 0.44, -0.17));
      const load = [0, 1, 2].map(i => { const c = oreChunk(0.04); c.position.set(0, 0.58 + i * 0.05, -0.17); b.add(c); return c; });
      return { root: sk.root, body: b, load, walk: sk.walk, mine: sk.mine, speed: 1.4, height: 1.0 };
    },
  },
];
