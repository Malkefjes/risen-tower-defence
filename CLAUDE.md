...
- **Ore look:** Erik picked B1 (ore body) from `mockups/ore-b/`, without the crater: a node sitting on the snow, two kinds with the same shape: stone (plain grey, no flecks) and metal (whole node steel-silver). `createOreNode(n, seed, kind)`. Model in `src/render/ore.ts` (`createOreNode(n, seed)`, `setAmount` shrinks it). Ore playground: `mockups/ore-play/` (https://claude.ai/artifact/WoEZJHDMM4D1tzaozyKevD): 2×2 and 3×3 nodes, hold left mouse to mine (12 ore/s: ~1.1 s per stage on a 2×2, ~2.8 s on a 3×3), nodes are solid. Nodes break in three stages like Rust (outer rocks at 2/3, middle at 1/3, core at 0, with chunk bursts), not a smooth shrink; respawn after 20 s in the playground (respawn in the game is a later decision).
