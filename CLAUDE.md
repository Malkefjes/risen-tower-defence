...
- **Next:** Erik picks a ship. He liked the Lander (`mockups/ship/`, https://claude.ai/artifact/NPGkLrVg6Pa4hjFpzexyk7) but wants it to read more as a rocket; rocket-like takes in `mockups/rocket/` (https://claude.ai/artifact/VXYi9z5m4XzL1zLuMW9fLr). Then M1a in three parts (avatar + ship, ore, fabricator + build range), each described to Erik before building. Work one step at a time: describe exactly what you'll build next and wait for Erik's feedback before implementing.
