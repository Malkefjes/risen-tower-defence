...
- **Hotbar (ore playground, into the game next):** Rust/Minecraft-style, 6 slots at the bottom centre, the only inventory for now (`src/sim/inventory.ts`, `Hotbar`, tested). Slot 1 holds the multitool; keys 1–6 select; the tool only fires with the multitool selected. Ore arrives in whole chunks, one per stage that breaks off (a third of the node: stone 333/334/333, metal 167/166/167), never ticking up while mining; it stacks into slots (stack 1000, tops up its own stack, then the first empty slot); if the next chunk does not fit, mining does nothing (the node keeps its ore, the popup shows Full). The stone/metal counter was removed; a "+N" with the ore icon pops over the node when a stage breaks and fades after 1 s. Selected slot outlined in colony orange; no slot numbers (no control hints).
