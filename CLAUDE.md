...
- **Ore look:** mockup `mockups/ore/` (https://claude.ai/artifact/PwXfZjDr7kB2rQexNtggfs): three deposit styles set into the ground (A cracked plate with veins, B crater, C tilted strata), each 2×2 and 3×3, gold ore. Erik chose B (crater) but it read as crystals; raw-ore takes in `mockups/ore-b/` (https://claude.ai/artifact/HqvxXbX5y9XA9Mb7btp5Jh): B1 ore body, B2 rubble bed, B3 mine face. Waiting on Erik's pick.
