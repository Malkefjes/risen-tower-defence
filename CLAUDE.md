...
- Caves: look A (stacked rock outcrop, dark mouth, icicles; no violet, no glow). Cliffs: look B (tilted slabs with even snow). Enemy: the leaper (dark red #1e0406 with bone-white blades and stinger, no eyes) is the current Grunt and will be replaced. The Brute is locked: Erik's Colossus golem (`render/colossus.ts`, cut into parts and coloured by rule), mined-stone grey, size 2, speed 1, 80 HP, alone, thin red outline, HP bar always; try enemies in `mockups/playground` (the real game on a tight maze, spawn on demand). Smelter: the round furnace with a framed molten window.
