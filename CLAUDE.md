...
- **Hotbar (ore playground, into the game next):** Rust/Minecraft-style, 6 slots at the bottom centre, the only inventory for now (`src/sim/inventory.ts`, `Hotbar`, tested). Slot 1 holds the multitool; keys 1–6 select; the tool only fires with the multitool selected. Ore arrives in whole chunks, one per stage that breaks off (stone 200, metal 100 per stage), never ticking up while mining; it stacks into slots (stack 1000, tops up its own stack, then the first empty slot); if the next chunk does not fit, mining does nothing (the node keeps its ore, the popup shows Full). The stone/metal counter was removed; a "+N" with the ore icon pops over the node when a stage breaks and fades after 1 s. Ore icons are the node's last stage (its core rock) rendered from the real model at startup with the world's evening lights (sun ×1.9, sky ×1.05 so they pop on the dark hotbar; the metal icon alone gets a polished copy of its material and a soft room reflection so it reads silver) and the sun's shadow on (invisible) snow, faded at the edges (`oreIcons()` in `src/render/icons.ts`). Counts show as x200 (stack max 1000 for stone and metal). Selected slot outlined in colony orange; no slot numbers (no control hints).
