...
- **Movement playground:** `mockups/playground/` (https://claude.ai/artifact/3R3F7P5FTggg1WcXbAVQsR). Uses the real `src/sim/avatar.ts` (pure movement rules, tested in `tests/avatar.test.ts`) and `src/render/rig.ts` (rig model + `RigAnimator`, which blends idle/run/air/landing from motion). Erik is tuning the feel there (his values live in his browser's localStorage; he copies them with the panel's Copy values button); Erik's tuned values are now the defaults (speed 4 after in-game feedback, was 5 in the playground; accel 40, turn 20, jump 0.9 / rise 0.35, air control 0.5, stride 3, camera follow 4). No motion blur, ever (Erik dislikes it). Snow lives in world space and wraps around the camera area. Moving things must be drawn interpolated between 60 Hz sim ticks and animated per rendered frame, and the shadow camera snapped to its texel grid; otherwise the rig vibrates (fixed in the playground, carry into the game in M1a).
