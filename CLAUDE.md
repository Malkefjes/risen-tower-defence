...
- **World** (`sim/worldgen.ts`, `render/scenery.ts`): generated from a seed (localStorage `risen.world.seed`, default 1), about 224×224, no map edge. Zones around the landing site: clearing, pine forest belt, and windswept rocky highlands all around beyond (few pines, dead pines far out; the violet wastes are gone). Raised ground (plateaus, 1.35 high, cliff edges) blocks everything and grows a little more common the further out (about a fifth of the ground 40–80 cells out, about a third near the edge). Pines and dead pines never touch (a free tile all round). Trees and dead trees can be cut down: hold the multitool on one in reach (`chopTime`, 1.5 s), and its cell is open ground for the rest of the run (`World.fellTree`; a new run brings them back). The view hides a cut tree by collapsing its vertices in the baked chunk (`bakeStatic` records each tree's vertices). Snow on the ground is always pure white; variety comes from bare patches with a cause (ice lakes, the landing scorch, windswept rock). Footprints fill back in. Scenery never moves and is baked per 16×16 chunk (`bakeStatic`).
...
- **Raids**: no Start wave button (except in god mode). A raid clock (10:00 grace, then 3:00 after each cleared raid); mining, building and smelting pull it closer but never into the fixed 60 s warning, when the active caves tremble, red chevrons show where it comes from, and the clock shows the raid's mix (each raid is planned ahead: `Game.raidPlan`, `raidMix`). Raids grow in a straight line (`raidBase` + `raidStep` a raid per cave, packs spread over `raidSpread` s), shared between the types by `share` (a type's first raid brings a whole pack); each type comes in from its `from` raid (Swarm 1, Runner 2, Brute 3); caves open up (one cave, another every `caveEvery` raids: 1, 1, 2, 2, 3…); HP grows +`enemyHpStep` (25%) of raid-1 HP a raid. Raid 1 is 12 Swarms; about 170 enemies by raid 10. Packs climb out a body length apart (`ENEMY_INFO.length`).
- **Restart** (top bar): a second click within 3 s starts a new run (the button asks, "Sure?"; no popup).
- **God mode** (top bar "God", remembered in localStorage `risen.god`): building, plating, growing and repairs cost nothing, upkeep stops, a Start raid button shows in calm. Free things sell for nothing. Phases in code are `"planning"` (calm) and `"wave"` (raid); the UI says Calm and Raid.
...
- Caves: look A (stacked rock outcrop, dark mouth, icicles; no violet, no glow). Cliffs: look B (tilted slabs with even snow). Enemies (the leaper is gone): the Brute is locked: Erik's Colossus golem (`render/colossus.ts`, cut into parts and coloured by rule), mined-stone grey, size 2, speed 1, 80 HP, alone, thin red outline, HP bar always. The Runner is locked: Erik's stone wolf (`render/wolf.ts`), size 1.5, speed 3 (24 HP since the balance pass, few and tough). The Swarm: Erik's Grumtooth (`render/grumtooth.ts`, 5 HP). All three share `render/stoneCreature.ts` (cutting, mined-stone colour, outline). Try enemies in `mockups/playground` (the real game on a tight maze, spawn on demand). Smelter: the round furnace with a framed molten window.
...
- **Balance numbers answer to how a raid feels, not only to what the towers can hold.** The first balance pass sized raids by capacity alone (the right towers holding at 80% of the curve's alloy), and because missiles wipe out bunched Swarms cheaply it sent 132 Swarms in raid 1: holdable, but unreadable. Every number needs an experience check too (how many enemies, from how many directions, can a new player follow them), and the first raids are for learning the game. Say the count out loud before shipping a balance change.
