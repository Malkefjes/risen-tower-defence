...
- **Next: M1a part 2 (ore).** Describe it to Erik before building: ore nodes, mining with a held key, carrying ore to the ship, ore replaces credits. Mining is hold left mouse button (Erik).
- **Hotbar (ore playground, into the game next):** Rust/Minecraft-style, 6 slots at the bottom centre, the only inventory for now (`src/sim/inventory.ts`, `Hotbar`, tested). Slot 1 holds the multitool; keys 1–6 select; the tool only fires with the multitool selected. Ore arrives in whole chunks, one per stage that breaks off (stone 200, metal 100 per stage), never ticking up while mining; it stacks into slots (stack 1000, tops up its own stack, then the first empty slot); if the next chunk does not fit, mining does nothing (the node keeps its ore, the popup shows Full). The stone/metal counter was removed; a "+N" with the ore icon pops over the node when a stage breaks and fades after 1 s. Ore icons are the node's last stage (its core rock) rendered from the real model at startup with the world's evening lights (sun ×1.9, sky ×1.05 so they pop on the dark hotbar; metal is polished (metalness 1, roughness 0.15) and mirrors a dim room with bright light strips (`stripRoom()` in `ore.ts`), so some facets flash bright: that is what reads as silver (a plain white room just looked white). World nodes get it via `polishMetal(renderer)` (envMap on the metal materials only, intensity 1.5; call it once with the world renderer, the game's view must too when ore lands); the icon renders its own copy at intensity 1) and the sun's shadow on (invisible) snow, faded at the edges (`itemIcons()` in `src/render/icons.ts`, which also renders the multitool icon side on from `createMultitool()` in `rig.ts`). Counts show as x200 (stack max 1000 for stone and metal). Selected slot outlined in colony orange; no slot numbers (no control hints).
