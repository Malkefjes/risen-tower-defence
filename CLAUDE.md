...
- **Enemies** (`sim/enemies.ts`, `render/leaper.ts`, `render/cave.ts`): each walker has a type (so far only the Grunt, drawn as the leaper). Leapers climb out of cave exits (the 3 nearest the ship are active) in packs of 3–5, one speed per pack, each on its own line within the tile (view only). They go for the nearest target (the ship or a smelter) and claw it from a neighbouring cell. Walls are slow obstacles: the flow field counts a wall as its chew time, so a maze gets walked and a full block gets chewed (at most 2 enemies per piece; a stone piece takes about 150 s). A piece breaks as a whole, with any tower on it. With nothing left to attack they burrow.
...
- Walls: stone look C (terraced courses); plated walls are the Armored deck (orange). Ship: the Rocket with the Reactor core. Turrets: Twin/Gatling (design B), now the Gun at 1×1 and 2×2.
...
- Caves: look A (stacked rock outcrop, dark mouth, icicles; no violet, no glow). Cliffs: look B (tilted slabs with even snow). Enemy: the leaper (dark red #1e0406 with bone-white blades and stinger, no eyes) is the current Grunt and will be replaced. The Brute is locked: Erik's Colossus golem (`render/colossus.ts`, cut into parts and coloured by rule), mined-stone grey, size 2, speed 1, 80 HP, alone, thin red outline, HP bar always. The Runner is locked: Erik's stone wolf (`render/wolf.ts`), size 1.5, speed 3, 4 HP, packs. The Swarm is being tried: Erik's Grumtooth (`render/grumtooth.ts`). All three share `render/stoneCreature.ts` (cutting, mined-stone colour, outline). Try enemies in `mockups/playground` (the real game on a tight maze, spawn on demand). Smelter: the round furnace with a framed molten window.
