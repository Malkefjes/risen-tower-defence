...
- **Hotbar (ore playground, into the game next):** Rust/Minecraft-style, 6 slots at the bottom centre, the only inventory for now (`src/sim/inventory.ts`, `Hotbar`, tested). Slot 1 holds the multitool; keys 1–6 select; the tool only fires with the multitool selected. Mined ore stacks into slots (stack 1000, tops up its own stack, then the first empty slot); a full hotbar mines nothing (the node keeps its ore, the popup shows Full). The stone/metal counter was removed; a "+N" with the ore icon floats over the node while mining, counts up, and fades 1 s after you stop. Selected slot outlined in colony orange; no slot numbers (no control hints).
