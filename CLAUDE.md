...
- **Movement playground:** `mockups/playground/` (https://claude.ai/artifact/3R3F7P5FTggg1WcXbAVQsR). Uses the real `src/sim/avatar.ts` (pure movement rules, tested in `tests/avatar.test.ts`) and `src/render/rig.ts` (rig model + `RigAnimator`, which blends idle/run/air/landing from motion). Erik is tuning the feel there; bake the tuned numbers into `defaultAvatarTuning` before M1a part 1.
