...
const open = (extra: Partial<MapDef> = {}): MapDef => ({
  name: "test", spawners: [[0, 0]], ship: [[10, 0]], rocks: [], trees: [], ...extra,
});
...
describe("pathfinding", () => {
  ...
  it("uses diagonals", () => {
    const f = computeField(new World(open({ ship: [[3, 3]] })));
  ...
  it("never cuts between two diagonal walls", () => {
    ...
    const w = new World(open({ ship: [[1, 1]] }));
  ...
  it("the world has no edge: enemies walk around a long wall", () => {
    const w = new World(open({ ship: [[0, 5]] }));
...
describe("placement", () => {
  ...
  it("refuses overlapping terrain, ship and rift", () => {
  ...
  it("may fully enclose the ship: enemies will chew through", () => {
    const g = planningGame(open({ spawners: [[6, 0]], ship: [[0, 0]] }));
    // Ring around the ship, leaving (1,-1) and (1,0) open.
  ...
  it("can't seal a path with terrain alone", () => {
    ...
    const g = planningGame(open({ spawners: [[6, 0]], ship: [[0, 0]], rocks }));
  ...
  it("pieces placed in calm lock when the raid starts; mid-raid placements lock at once", () => {
    const g = planningGame(open({ ship: [[20, 0]] }));
  ...
  it("enemies reroute when a wall appears on their route", () => {
    const g = planningGame(open({ spawners: [[0, 0]], ship: [[12, 0]] }));
    g.startWave();
